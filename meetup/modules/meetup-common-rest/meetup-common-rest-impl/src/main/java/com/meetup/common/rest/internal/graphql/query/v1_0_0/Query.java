package com.meetup.common.rest.internal.graphql.query.v1_0_0;
...
public class Query {
	 ...
	 *
	 * curl -H 'Content-Type: text/plain; charset=utf-8' -X 'POST' 'http://localhost:8080/o/graphql' -d $'{"query": "query {serviceDetailsByServiceId(serviceId: ___){}}"}' -u 'test@liferay.com:test'
	 */
	@GraphQLField
	public Object serviceDetailsByServiceId(
			@GraphQLName("serviceId") String serviceId)
		throws Exception {

		return _applyComponentServiceObjects(
			_serviceResourceComponentServiceObjects,
			this::_populateResourceContext,
			serviceResource -> serviceResource.getServiceDetailsByServiceId(
				serviceId));
	}

	@GraphQLName("ServicePage")
	public class ServicePage {

		public ServicePage(Page servicePage) {
			actions = servicePage.getActions();

			items = servicePage.getItems();
			lastPage = servicePage.getLastPage();
			page = servicePage.getPage();
			pageSize = servicePage.getPageSize();
			totalCount = servicePage.getTotalCount();
		}
		...
		@GraphQLField
		protected java.util.Collection<Service> items;
